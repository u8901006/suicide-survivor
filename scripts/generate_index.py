#!/usr/bin/env python3
"""Generate index.html listing all Suicide Survivor daily reports."""

import glob
import os
from datetime import datetime

html_files = sorted(glob.glob("docs/survivor-*.html"), reverse=True)
links = ""
for f in html_files[:30]:
    name = os.path.basename(f)
    date = name.replace("survivor-", "").replace(".html", "")
    try:
...
<title>Suicide Survivor · 自殺喪親文獻日報</title>
<style>
  :root {{ --bg: #0f0f1a; --surface: #1a1a2e; --line: #2a2a4a; --text: #e8e8f0; --muted: #8888aa; --accent: #6c63ff; --accent-soft: rgba(108,99,255,0.15); --success: #4ecdc4; }}
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  body {{ background: var(--bg); background-image: radial-gradient(ellipse at 20% 50%, rgba(108,99,255,0.08) 0%, transparent 50%), radial-gradient(ellipse at 80% 20%, rgba(78,205,196,0.06) 0%, transparent 50%); color: var(--text); font-family: "Noto Sans TC", "PingFang TC", "Helvetica Neue", Arial, sans-serif; min-height: 100vh; }}
...
<div class="container">
  <div class="logo">🪷</div>
  <h1>Suicide Survivor</h1>
  <p class="subtitle">自殺喪親文獻日報 · 每日自動更新</p>
  ...
  <footer>
    <p>Powered by PubMed + NVIDIA NIM (Nemotron 3) · <a href="https://github.com/u8901006/suicide-survivor">GitHub</a></p>
  </footer>
</div>
</body>
</html>"""
